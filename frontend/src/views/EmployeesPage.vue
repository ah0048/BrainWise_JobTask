<template>
    ...
    methods: {
      async fetchEmployees() {
        ...
        for (let employee of data) {
          ...
          employee.departmentName = departmentData.name; // Add department name to employee
        }
      ...
      async editEmployee(employee) {
        ...
        if (employee.status === "Application Received") {
          this.allowedStatusOptions = ["Interview Scheduled", "Not Accepted"];
        } else if (employee.status === "Interview Scheduled") {
          this.allowedStatusOptions = ["Hired", "Not Accepted"];
        } else {
          this.allowedStatusOptions = [];
        }

        // Prevent editing if status is "Hired" or "Not Accepted"
        this.canEdit = !(employee.status === "Hired" || employee.status === "Not Accepted");
  ...
  const token = localStorage.getItem("token");
    ...
    if (response.ok) {
      ...
      onCompanyChange() {
        this.filteredDepartments = this.departments.filter(dept => dept.company === this.form.company);
      },
    },
  };
  </script>
